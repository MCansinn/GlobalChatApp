...
public class ChatHub : Hub
{
    ...
    public override async Task OnConnectedAsync()
    {
        ...
        var username = Context.User.Identity.Name; // Kimlik doğrulama ile alınıyor.
    ...
    public async Task SendMessage(string encryptedMessage)
    {
        ...
        {
            await Clients.Caller.SendAsync("Error", "Eşleşme bulunamadı.");
        }
    }
    ...
    public async Task BlockUser()
    {
        ...
        {
            await Clients.Caller.SendAsync("Error", "Engellenecek eşleşme bulunamadı.");
        }
    }
    ...
    // Yeni: Tepki gönderme metodu
    public async Task SendReaction(string messageId, string reaction)
    {
        if (Connections.TryGetValue(Context.ConnectionId, out var currentUser) &&
            !string.IsNullOrEmpty(currentUser.CurrentChatPartner))
        {
            var partnerConnection = currentUser.CurrentChatPartner;
            // Hem göndericiye hem de alıcıya tepki gönderiyoruz.
            await Clients.Client(partnerConnection).SendAsync("ReceiveReaction", messageId, currentUser.Username, reaction);
            await Clients.Caller.SendAsync("ReceiveReaction", messageId, currentUser.Username, reaction);
        }
        else
        {
            await Clients.Caller.SendAsync("Error", "Eşleşme bulunamadı.");
